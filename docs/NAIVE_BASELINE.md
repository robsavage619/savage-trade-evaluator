# Naive Baseline Evaluator: Design
...
- `$_per_war` = 2024-dollar-adjusted FA market price per WAR (about $8M/WAR in the current FA market; will be re-derived from our data)
...
2. **Variance.** Point estimate only, no posterior. D-13 says we want distributions, and the naive model gives means.
...
- **Q-01** (trade scope cutoff): for now apply the model to *every* trade event. Filter when characterizing residuals, so Q-01 drops out of the residual distribution shape.
