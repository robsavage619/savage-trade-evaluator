# Baseline and first ablations (R-01 to R-09)

Establishing a predict-zero baseline, finding that OLS overfits where a Bayesian
fit holds, and running the first feature ablations. Most of these are nulls. The
one win is R-05, the pitcher dev-fit feature.

Part of the [research log](README.md). Entries run oldest first.

---

## [2026-05-15] R-01: Phase 2 V0 naive baseline backtest against realized-WAR ground truth
...
| 2 | Yelich MIA to MIL 2018 | +21.50 (MIL) |
| 3 | Arrieta PIT to CHC 2013 | +20.78 (CHC) |
| 4 | Chapman OAK to TOR 2022 | +20.01 (TOR) |
| 5 | Cliff Lee CLE to PHI 2009 | +19.50 (PHI) |
| 6 | Cliff Lee SEA to TEX 2010 | +18.92 (TEX) |
| 7 | Trea Turner and Scherzer WSH to LAD 2021 | +18.54 (LAD) |
| 8 | Mookie Betts BOS to LAD 2020 | +16.84 (LAD) |
| 9 | Verlander DET to HOU 2017 | +16.75 (HOU) |
| 10 | Holliday OAK to STL 2009 | +16.11 (STL) |
...
**Setup.** Features: `receiver_prior_year_war`, `receiver_dev_fit_pitching`, `receiver_dev_fit_hitting`. Target: realized surplus. Train through 2020, test 2021+. OLS via `numpy.linalg.lstsq`. 3,735 train rows / 1,730 test rows.
...
The pitching-dev-fit coefficient was the strongest at +0.026, which narratively matches the MVP Machine thesis. Prior-year-WAR coefficient was slightly negative (counterintuitive; likely confound from deadline-sellers being high-WAR teams).

**Interpretation.** Trades are near-zero-sum in expectation; surplus has high variance that a 3-dim linear fit can't capture. The negative point-prediction result is *informative*: it justifies the multilevel Bayesian framing with strong priors (the V2 architecture).
...
**Interpretation.** The Bayesian posterior is well-calibrated even when it can't beat zero on point predictions, which is exactly the D-13 inductive bias. Strong priors shrink toward predict-zero where signal is absent. Confidence: high.
...
**Setup.** Same data, expanded `FEATURE_COLUMNS` from 3 to 5 features. Train through 2020, test 2021+. Both models fit on the matched 1,730-row test set.
...
| Bayesian 5-feature | 1.2085 | 1.1635 | -0.83% (CRPS *improved* on R-03's -1.06%) |
...
**Interpretation.** Empirically validates the D-12/D-13 inductive bias: strong priors regularize toward predict-zero when features are weak; OLS without that protection catastrophically overfits. Bayesian CRPS even *improved* slightly when the features were added, because the priors shrank their weak coefficients without amplifying noise. Confidence: high.
...
Posterior `tau_team` rose from 0.08 to 0.14 (the model finds team-specific signal). Posterior `sigma` fell from 2.83 to 2.43 (less irreducible noise).

Feature spot-check matches the literature: LAD 2018 = +16.17, MIN 2018 = -25.0, HOU 2019 = +3.33 (post-Verlander/Cole entry).

**Interpretation.** First genuine beat-zero on CRPS in this project. The MVP Machine Ch 9 thesis is empirically detectable at the team-season aggregate level. Confidence: high. Note: rhat warnings on short chains. Production claims would want 4 chains of 2000 samples.
...
## [2026-05-16] R-06: Hitter dev-fit ablation. Feature contributes ~0 on the matched subset
...
**Setup.** `scripts/ablation_hitter_feature.py`. Pulled the *matched* 911-row test subset (rows with ALL 7 features non-null, 2021+). Fit two Bayesian variants on the same training rows: (A) all 7 features; (B) drop `receiver_org_hitter_xwoba_jump_3yr`. Same seed (137), 2 chains, 1000 tune and 1000 draws each, same prior structure.
...
- R-07 candidate: try a per-acquired-player operationalization (couple team k-jump history with the specific pitcher's pre-trade stuff percentile), which captures interaction effects.
- R-08 candidate: try a per-coach operationalization (use the `coaches` table: does a *specific hitting coach's* trade-acquired-player xwOBA jump history predict their next acquisition's surplus?).
...
**Question.** R-06 found the team-aggregate hitter dev-fit feature was silent. Hypothesis (D-19): the Ch 5 thesis lives at a per-coach level. When Tim Hyers moves from LAD assistant to BOS head to TEX head, his coaching signal moves with him, and a team aggregate gets diluted across coach turnover. Does a per-coach trailing-3yr xwOBA-jump feature recover the signal?

**Setup.** New feature `coach_hitter_xwoba_jump_3yr`: for each (team, season) the trailing-3yr mean xwOBA jump for hitters this team acquired, but the aggregation is **per-COAT (hitting coach)**, not per-team. The feature follows the coach across team moves. Shifted forward 1 season so it reflects pre-trade-year knowledge. Matched-subset A/B via `scripts/ablation_coach_feature.py`. 702 test rows (rows with all 8 features non-null).
...
2. The "who they acquired" confound (BOS acquires stars, not reclamation projects) dominates "who's coaching them." Selection-on-gains corrupts both team and per-coach aggregations the same way.

Also notable: both fits' CRPS is **worse** than predict-zero on this subset (-3.15% and -3.09%), unlike the 911-row subset where the 7-feature fit was +1.07%. The 702-row subset (post-2015 trades with a documented hitting-coach assignment and acquisition history) has a *lower* predict-zero CRPS (1.057 vs 1.105). These trades are nearer zero on average and harder to beat. Same sample-shrinkage confound R-06 named, recurring at a different cut.
...
- The hitting side may need either (a) much more data, on a scale of decades, or (b) a fundamentally different feature shape, namely player-level swing-change data we don't have from public sources.
...
| Cost-controlled-surplus expectation | FV-to-WAR table times years of control remaining | Craig Edwards' published FG tables; need to source |
...
**Setup.** Ingested 46,126 draft picks 1990-2024 (`ingest/draft.py`, schema v11). Built `trade_pedigree` view (per trade-event-per-receiver: MIN/AVG/count of pick_number across acquired players). Joined as `receiver_best_draft_pick` into `trade_with_context`. Matched-subset A/B: 9-feat (with) vs 8-feat (without) on the 763-row subset where all 9 features are non-null. Train pre-2021, test 2021-2024. PyMC, seed=137, 1000 tune plus 1000 draws across 2 chains.

**Result.** Delta CRPS = +0.0008 (essentially zero, slightly *worse* with the feature). Delta MAE = +0.0033. Both models score ~+4.5% CRPS against predict-zero on this subset, the same regime as R-06/R-07. The new feature does not move the needle. See `scripts/ablation_draft_feature.py`.

Pressly sanity check passed: HOU side (received Pressly, MLB pick #354 / 2007 R11) has `receiver_best_draft_pick = 354`; MIN side (received Alcala and Celestino, both international FAs) is NULL, which is correct.
...
2. **Pick number is too coarse a pedigree proxy.** A 1st-round signability case (think Mark Appel #1 overall) and a Mike Trout #25 are both "first-round picks" but worth radically different things at trade time. MLB Pipeline top-100 *ranks* would discriminate better but were blocked by lazy-load at the time. FV grades would be better still.
