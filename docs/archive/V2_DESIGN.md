# V2 model design (archived)

> **Status: superseded.** V2 shipped and was then replaced by V3. R-33, R-34,
> and R-35 found that the multilevel (team, regime) structure below added no
> predictive signal over a flat model, so V3 dropped it. The production model
> is `modeling/v3.py`. Code for V2 still lives under `modeling/v2/` and the
> `ste v2` CLI still runs, but nothing in the product path calls it.
>
> Kept because the feature design, the outcome definitions, and the
> credibility bar below all carried forward into V3.
...
For any proposed trade `(team_A, team_B, players A to B, players B to A)`, V2 outputs **joint posterior distributions** over four outcome metrics per receiving team:
...
All features are within-team-variation per D-24. Static team-only signals failed in R-06/07/09/14.
...
- **Smoke test**: calibration at 90% or better on held-out 2021-2024; at least 1 credible feature per outcome at the D-26 threshold

### Credibility framework (D-26)

A feature is reported as "credibly real" iff its posterior has **a 90% CI excluding zero AND at least 95% directional mass**. This is the bar that surfaced R-19's three features and R-22's k_trajectory finding.
...
5. **Distribution smoke test**: coverage at 90% or better on the test set; at least 1 credible feature per outcome
6. **R-33 V2 baseline ablation**: against V1 on the same data. Does V2 add credible features? (It did not. See the status note at the top.)

## Open follow-ups (V2.1+)

- **Joint vector outcome** instead of four parallel models (captures correlations across metrics)
- **League $/WAR derived from FA signings** per season, not hardcoded
- **Arb-eligible / FA-eligible status** as features (from Spotrac `status` field)
- **Pre-trade contract length** (years remaining)
- **Trade-counterparty features**: what is team_B giving up? Only team_A's receive side is modeled.
