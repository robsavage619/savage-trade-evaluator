# Data source probe, 2026-05-16 (archived)

> **Status: historical.** This is a point-in-time sweep. Several of its blocked
> verdicts no longer hold: FanGraphs leaderboards and The Board are ingested via
> a Firecrawl stealth proxy, Spotrac is ingested, and MLB Pipeline prospects are
> ingested. For current status read `docs/STATS_CATALOG.md` or run
> `uv run ste catalog`. Kept for the reasoning that produced the ingest plan.
...
| errors | 1 | Sean Lahman host connect-error |

## Ingests added in this pass
...
- **Chadwick gives us real birth dates.** 96.6% of our 23,622 bWAR players now have birth_year/month/day. Replaces the years-since-debut age proxy across all R-13/R-25/R-27/R-29/R-30 work. Also provides a clean MLBAM / Retro / BRef / Fangraphs ID cross-walk for future joins.
- **Catcher framing** was a previous gap (pybaseball CSV-parse failed). Direct fetch from Savant CSV works. Lets us decompose pitcher xERA improvements into real gains vs catcher-framing artifact, which directly addresses an R-22 caveat.
...
| Source | URL | Status |
|---|---|---|
| FanGraphs leaders (HTML) | `fangraphs.com/leaders.aspx` | 403, Cloudflare-gated (D-25 held at the time) |
...
## URL corrections (404s, then fixed)
...
| MLB Trade Rumors int'l tracker 2024 | Posted-URL guess | Annual posts move; would need a site search |
...
1. **MLB Trade Rumors annual international signing tracker.** Find the correct URL for each year, scrape names and bonuses. Replaces the current "post-1995 not in draft_picks" proxy with real attribution.
...
6. **fWAR via a FanGraphs alt route.** Try `r.fangraphs.com` or archive.org cached snapshots.
...
**Agent finding:** the existing `ingest/front_office.py` scraper that pulls 2010-2024 from Baseball Reference per-season team pages can be **iterated backward** with no code changes. Every per-season BR team page (1980+) has the same `General Manager:` field, which is far simpler than the Wikipedia or Wikidata routes we tried.

Confirmed for `baseball-reference.com/teams/LAD/2008.shtml`, which carries "Ned Colletti" in the team metadata. Same path the existing BR scrape uses for 2010+, just iterate backward.
...
- **Spotrac player pages.** Accessible. For example `spotrac.com/mlb/los-angeles-dodgers/mookie-betts-23391/` returns real HTML with contract values inline (Ohtani $700M, Betts $365M, Yamamoto $325M, Freeman $162M all visible).
...
Cot's, Baseball America, and MLB Trade Rumors remain genuinely blocked: they fail with 403, require accounts, or have aggressive bot detection. (MLB Trade Rumors was later ingested; see `docs/STATS_CATALOG.md`.)

### Original blocked finding (kept for context; applies to the WebFetch tool only)
...
These domains require an explicit policy decision to add. The agents agreed the best allowlist addition would be **`docs.google.com` plus `baseballprospectus.com`**, because Cot's publishes contract data as public Google Sheets and the CSV-export trick works without scraping.
...
If int'l signing depth is needed, Baseball America is the better target, but it is both paywalled and not allowlisted.
