# Decision Drafts — Trust + Explainability Release (2026-07-05)

ADR entries for Steps 2–11. Append to `~/Vault/savage_vault/wiki/trade-eval--decisions.md` as D-53 through D-56.
...
salary over a multi-year window, inflating surplus by approximately N× (where N = window length).

**Impact:** Prior code overstated cost-controlled surplus for multi-year windows. With a 4-year
window and $10M/yr salary, prior output was $10M cost vs. $40M correct — a 4× error. The fix also
...
**Empirical calibration deferred:** No regression test against ground-truth deal economics —
...
The midpoint shift from 89 → 86 reflects that 12 teams now qualify per league (vs. 10 pre-2022), so
...
**NO-GO** for production use. On sparse rows (≥25% missing features), MI widened intervals as
...
ignoring correlations (e.g., exit velocity ↔ barrel rate). With 8+ missing Statcast features,
...
That is a D-38 experiment — requires a new fit or a separate covariance cache, and a new GO/NO-GO
validation pass.

---

## D-56 — Attribution + coverage layer in scenario payloads (2026-07-05)
...
imputed_features list, observed_fraction, and a letter grade (A ≥0.8, B ≥0.6, C ≥0.4, D else).
...
feature row identically to `predict()` — clip, subtract means, divide by stds; NaN → 0 (zero
contribution in z-space by construction). Computes `contribution_i = beta_mean_i × x_z_i × y_std`
...
uncertainty band is `√n·1.2` heuristic, not a model posterior.
