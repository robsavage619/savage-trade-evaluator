...
import { FlaskConical, TrendingDown, Map as MapIcon, Wrench, CheckCircle2, XCircle, Microscope } from 'lucide-react'
import thesesData from '../data/model/theses.json'
...
type Thesis = {
  ...
  priority: 'investigate' | 'monitor' | 'noise'
}
...
const PRIORITY_META = {
  investigate: { label: 'Investigate', color: 'text-accent-400 border-accent-500/30 bg-accent-500/8' },
  monitor: { label: 'Monitor', color: 'text-ink-300 border-ink-600 bg-ink-800' },
  noise: { label: 'Noise', color: 'text-ink-500 border-ink-700 bg-ink-850' },
}
...
function ThesisCard({ thesis }: { thesis: Thesis }) {
  const meta = PRIORITY_META[thesis.priority]
  return (
    <div className="card flex flex-col gap-4 p-6">
      <div className="flex items-start justify-between gap-4">
        <p className="text-[15px] font-semibold leading-snug tracking-tight text-ink-100">
          {thesis.headline}
        </p>
        <span className={`shrink-0 rounded border px-2 py-0.5 text-[9.5px] font-semibold uppercase tracking-[0.14em] ${meta.color}`}>
          {meta.label}
        </span>
      </div>

      <p className="text-[13px] leading-relaxed text-ink-300">{thesis.mechanism}</p>

      <p className="border-l-2 border-accent-500/30 pl-4 text-[12px] italic leading-relaxed text-ink-400">
        {thesis.analyst_note}
      </p>

      <div className="flex flex-wrap items-center gap-2 pt-1">
        <span className="mono text-[10px] text-ink-500">{thesis.evidence.feature}</span>
        <span className="text-ink-700">·</span>
        {thesis.evidence.outcomes.map((o) => (
          <span key={o} className="mono rounded bg-ink-800 px-1.5 py-0.5 text-[10px] text-ink-400">{o}</span>
        ))}
        <span className="text-ink-700">·</span>
        <span className={`mono text-[11px] font-semibold ${DIRECTION_COLOR[thesis.evidence.direction]}`}>
          {thesis.evidence.direction === 'positive' ? '+' : '−'}{Math.abs(thesis.evidence.mean_beta).toFixed(3)} β
        </span>
        <span className="text-ink-700">·</span>
        <span className="mono text-[10px] text-ink-400">{(thesis.evidence.mean_mass * 100).toFixed(1)}% mass</span>
      </div>
    </div>
  )
}
  ...
  return (
    <main className="relative">
      ...
      <div className="border-b border-ink-800">
        ...
        {/* Station 6 — Analyst Discoveries */}
        {(thesesData as { theses: Thesis[] }).theses.length > 0 && (
          <Station index={6} eyebrow="Pattern Recognition · data-driven" icon={Microscope}
            title="What the model found that nobody asked it to find">
            <p className="mb-6 max-w-2xl text-[14px] leading-relaxed text-ink-300">
              These findings emerged from a full sweep of all 8 outcome metrics against every available feature.
              No hypothesis was pre-registered. The algorithm surfaced them; a 20-year veteran of sports analytics
              assessed which ones are worth investigating.
            </p>
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              {(thesesData as { theses: Thesis[] }).theses.map((t) => (
                <ThesisCard key={t.id} thesis={t} />
              ))}
            </div>
            <p className="mt-4 text-[11px] text-ink-500">
              Generated by sweeping {(thesesData as { generated_at: string; n_findings_considered: number }).n_findings_considered} credible feature findings
              across 8 independent outcome metrics (2010-2020 train, 2021-2024 holdout).
            </p>
          </Station>
        )}
