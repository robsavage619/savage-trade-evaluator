...
export function PlayerPicker({ team, onPickTeam, onAdd, selectedIds, title, hint }: Props) {
  ...
  const list = useMemo(() => {
    ...
    const flat: Array<{ player: CurrentPlayer; team: CurrentTeam }> = []
        ...
        <div className="mt-2 flex items-center gap-1">
          {(['all', 'pitcher', 'hitter'] as PosFilter[]).map((p) => (
            <button
              ...
              className={`rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-wider ${
                pos === p ? 'border-accent-500/60 bg-accent-500/10 text-accent-300' : 'border-ink-700 text-ink-400 hover:text-ink-200'
              }`}
            >{p}</button>
          ))}
          ...
          {(['all', 'active', 'injured'] as StatusFilter[]).map((s) => (
            <button
              ...
              onClick={() => setStatus(s)}
              className={`rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-wider ${
                status === s ? 'border-accent-500/60 bg-accent-500/10 text-accent-300' : 'border-ink-700 text-ink-400 hover:text-ink-200'
              }`}
            >{s}</button>
          ))}
        </div>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mono text-[10px] tabular text-ink-400">
                        ...
                        {p.last_salary != null ? ` · ${fmtMoney(p.last_salary)}` : ''}
                      </div>
                    </div>
            ...
            {players.map((p) => (
              <li key={p.mlb_player_id} className="flex items-center justify-between gap-2 rounded-md border border-ink-700 bg-ink-800/40 p-2 hover:border-ink-600">
                <Link to={`/player/${p.mlb_player_id}`} className="min-w-0 flex-1">
                  <div className="truncate text-[12.5px] font-semibold text-ink-100 hover:text-accent-300">{p.name}</div>
                  <div className="mono text-[10px] tabular text-ink-400">
                    {p.position_abbr ?? '—'}
                    {p.age != null ? ` · ${p.age}y` : ''}
