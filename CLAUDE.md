...
- **FanGraphs leaderboards ARE ingested** (2010–2024) via Firecrawl stealth proxy → `fangraphs_batting_leaders` (20,503 rows) + `fangraphs_pitching_leaders` (11,425 rows), cache under `data/fangraphs_cache/`. The old "blocked" note referred to live pybaseball scraping (still true: Cloudflare blocks pybaseball/curl_cffi/cloudscraper). Don't try pybaseball; the DB tables already have the data. **FanGraphs The Board (prospect FV grades) is also ingested** — 2017–2024 via `scripts/parse_fg_prospects.py` → `prospect_rankings` table → `trade_acquired_prospect_fv` view. For forward scoring (2025+) use TJStats instead (MLBAM IDs, no bridging needed).
