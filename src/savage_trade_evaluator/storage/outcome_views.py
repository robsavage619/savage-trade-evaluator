...
VIEW_STATEMENTS: tuple[str, ...] = (
    ...
    -- A4 fix: join on FG-id bridge (chadwick) where available; fall back to
    -- name-norm only when chadwick lacks a fangraphs_id for that player.
    -- Average over MATCHED rows only (no COALESCE to 0) so the model can
    -- distinguish "no prospects received" from "prospects with unknown FV."
    WITH name_norm AS (
        SELECT mlb_player_id,
               regexp_replace(lower(trim(player_name)), '[^a-z0-9 ]', '', 'g')
                   AS name_norm_key
        FROM trade_player_unified
        WHERE mlb_player_id IS NOT NULL
    ),
    ranked AS (
        -- ID-bridge path (preferred): prospect_rankings.fangraphs_player_id
        -- → chadwick_register.fangraphs_id → mlb_player_id
        SELECT
            tpu.trade_event_id,
            tpu.trade_season,
            tpu.to_team_bref     AS receiver_bref,
            pr.fv,
            'id_bridge'          AS match_method
        FROM trade_player_unified tpu
        JOIN chadwick_register cr
            ON cr.mlb_player_id  = tpu.mlb_player_id
            AND cr.fangraphs_id  IS NOT NULL
        JOIN prospect_rankings pr
            ON pr.fangraphs_player_id = cr.fangraphs_id
            AND pr.rank_year          = tpu.trade_season
        WHERE tpu.to_team_bref IS NOT NULL

        UNION ALL

        -- Name-norm fallback: only for players where chadwick has no fangraphs_id
        SELECT
            tpu.trade_event_id,
            tpu.trade_season,
            tpu.to_team_bref     AS receiver_bref,
            pr.fv,
            'name_norm'          AS match_method
        FROM trade_player_unified tpu
        JOIN prospect_rankings pr
        ...
        LEFT JOIN chadwick_register cr
            ...
            AND cr.fangraphs_id IS NOT NULL
        WHERE tpu.to_team_bref IS NOT NULL
          AND cr.mlb_player_id IS NULL
    ),
    deduped AS (
        -- Guard against double-counting if both paths match the same prospect.
        SELECT DISTINCT trade_event_id, trade_season, receiver_bref, fv, match_method
        FROM ranked
    )
    SELECT
        d.trade_event_id,
        d.receiver_bref,
        AVG(d.fv)::DOUBLE                          AS receiver_acquired_avg_fv,
        MAX(d.fv)::INTEGER                         AS receiver_acquired_max_fv,
        COUNT(*)::INTEGER                          AS n_acquired_prospects,
        SUM(CASE WHEN d.match_method = 'id_bridge' THEN 1 ELSE 0 END)::INTEGER
                                                   AS n_id_bridge_matches
    FROM deduped d
    GROUP BY d.trade_event_id, d.receiver_bref
    """,
)
...
def create_all(conn: duckdb.DuckDBPyConnection) -> None:
    ...
    Args:
        conn: Open DuckDB connection. Requires transactions, bwar_batting,
            bwar_pitching, teams, trade_movements, chadwick_register, and
            prospect_rankings to already exist.
    """
